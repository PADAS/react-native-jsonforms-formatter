  ...
  switch (uiField.type) {
    ...
    case 'COLLECTION':
      ...
      if (property.type === 'array' && property.items?.properties) {
        control.options!.detail = generateCollectionUISchemaInternal(property, uiField);
      }
      break;
...
export const createSectionLayout = (
  ...
  section.leftColumn.forEach(item => {
    if (item.type === 'field') {
      const element = fieldControls.find(el => el.scope === `#/properties/${item.name}`);
      if (element) {
 ...
 */
export const getVisibleFields = (schema: V2Schema): Array<{ name: string; property: V2Property; uiField: V2UIField }> => {
  const visibleFields: Array<{ name: string; property: V2Property; uiField: V2UIField }> = [];
...
): Record<string, Array<{ name: string; property: V2Property; uiField: V2UIField }>> => {
  const grouped: Record<string, Array<{ name: string; property: V2Property; uiField: V2UIField }>> = {};
...
const generateCollectionUISchemaInternal = (
  ...
  if (collectionProperty.type !== 'array' || !collectionProperty.items?.properties) {
    throw new Error('Collection property must be an array with item properties');
  }

  const itemProperties = collectionProperty.items.properties;
  const itemControls: JSONFormsControl[] = [];

  // Helper function to create control for a field
  const createItemControl = (fieldName: string, property: V2BaseProperty): JSONFormsControl => {
    const control: JSONFormsControl = {
      type: 'Control',
      scope: `#/properties/${fieldName}`,
    ...
    if (uiField.leftColumn) {
      uiField.leftColumn.forEach(fieldName => {
        if (itemProperties[fieldName]) {
          itemControls.push(createItemControl(fieldName, itemProperties[fieldName]));
        }
      });
    }
